...
In some scenarios, {ls} need to persist data to disk: 
...
<2> The `main` pipeline generates fail events to DLQ. Checkout the pipeline definition in the next session.
...
<2> The pipeline use {logstash-ref}/plugins-inputs-dead_letter_queue.html[dead_letter_queue input plugin] to consume DLQ events. Add filter plugins to fix error causing fail insertion, for example, mapping error.

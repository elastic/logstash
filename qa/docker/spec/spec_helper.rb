...
def get_pipeline_stats(container, pipeline='main')
  make_request(container, "curl -s http://localhost:9600/_node/stats/pipelines/#{pipeline}").dig('pipelines', pipeline)
end

def get_plugin_info(container, type, id, pipeline='main')
  pipeline_info = make_request(container, "curl -s http://localhost:9600/_node/stats/pipelines/#{pipeline}")
...
def logstash_available?(container)
  response = exec_in_container_verbose(container, 'curl -s http://localhost:9600')
...
def exec_in_container_verbose(container, command)
...
def exec_in_container(container, command)
  exec_in_container_verbose(container, command)[:stdout].join.chomp.strip
end

...
if base_logstash_version == "LOCAL"
  ...
  begin
    result = File.read("Gemfile.jruby-3.4.lock.release")
  rescue => e
  ...
  uri = URI.parse("https://raw.githubusercontent.com/elastic/logstash/v#{base_logstash_version}/Gemfile.jruby-3.4.lock.release")
...
FileUtils.rm_f("Gemfile.jruby-3.4.lock.release")
...
# rename file
puts "Finishing up.."
FileUtils.mv("Gemfile.lock", "Gemfile.jruby-3.4.lock.release")

`git checkout -- Gemfile.template`

puts `git diff Gemfile.jruby-3.4.lock.release`
...
`git commit Gemfile.jruby-3.4.lock.release -m "Update #{allow_bump_for} plugin versions in gemfile lock"`

...
logstash/{plugintype}s/base and logstash/namespace:
...
[float]
=== `class` Declaration
A {plugintype} plugin class should be a subclass of LogStash::{pluginclass}::Base:
...
If you set `config_name` with a value of "{pluginname}" in your plugin code,
...
[TIP]
====
For help learning about tests and testing, look for files like
`pluginname_spec.rb` in the
...
spec/{plugintype}s/
----------------------------------
directory of several other similar plugins.
====

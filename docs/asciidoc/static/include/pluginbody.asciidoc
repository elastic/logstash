...
subject to code review for scope and completeness.

To begin the acceptance process for migrating your plugin to logstash-plugins,
simply create a new https://github.com/elasticsearch/logstash/issues[issue] in
the Logstash repository. When complete, we will facilitate the move to the
logstash-plugins organization using the recommended

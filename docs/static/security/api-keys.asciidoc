...
I think we should start with "API keys are tied to the cluster they were created
in", and then go on to explain that in a single cluster a key can be shared for
ingestion+monitoring purposes, while a cluster production cluster + monitoring

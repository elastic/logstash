...
Note that data may be lost if an abnormal shutdown occurs before the checkpoint file has been committed.  
...
`queue.max_bytes`:: The total capacity of _each queue_ in number of bytes. The
default is 1024mb (1gb). 
The cumulative value (total of `queue.max.bytes` for _all_ queues) should be lower than the capacity of your disk. 
+

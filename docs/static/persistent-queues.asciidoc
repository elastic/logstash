...
[[pq-benefits-limitations]]
==== Persistent queue benefits and limitations
...
`queue.type`:: Specify `persisted` to enable persistent queues. By default, persistent queues are disabled (default: `queue.type: memory`).
`path.queue`:: The directory path where the data files will be stored. By default, the files are stored in `path.data/queue`. 
`queue.checkpoint.writes`:: Controls when the checkpoint file gets updated. 
See <<durability-persistent-queues>>.
...
to `1` ensure maximum durability, but can severely impact performance.
...
[[working-with-pqs]]
==== Working with persistent queues

[[troubleshooting-pqs]]
===== Troubleshooting persistent queues

//If you see an error message resembling this one, you likely have a persistent queue problem.
//:message=>"java.io.IOException: Page file size is too small to hold elements
// Here are some tips to help you identify and resolve the problem.
 
[[draining-pqs]]
===== Draining the queue

//From https://www.elastic.co/guide/en/logstash/6.8/upgrading-logstash-pqs.html#drain-pq

...
* Can be useful for Logstash deployments that need large buffers.
You can enable persistent queues to buffer events on disk in a buffered publish-subscriber model without a message broker such as Redis, RabbitMQ, or
Apache Kafka). 
...
If you see an error message similar to this one, you likely have a persistent queue problem.

```
message=>"java.io.IOException: Page file size is too small to hold elements
```

This error indicates that the head page (the oldest in a folder and the one with lowest page id) has a size < 18 bytes, the size of a page header.


To resolve the issue: 

. Identify the queue (or queues) that may be corrupt.
. Stop Logstash.
...
Run the `pqcheck` utility to identify which persistent queue--or queues--have been corrupted. 

From LOGSTASH_HOME, run:

[source,txt]
-----
bin/pqcheck <queue_folder>
-----

where `<queue_folder>` is the fully qualified path to the persistent queue location.
...
pageNum=1, firstUnackedPageNum=0, firstUnackedSeqNum=239675, minSeqNum=239675,  elementCount=218241, isFullyAcked=no <1> <2>
...
checkpoint.0, fully-acked: NO,<2> page.0 size: 67108864 
pageNum=0, firstUnackedPageNum=0, firstUnackedSeqNum=176126, minSeqNum=1, elementCount=239674, isFullyAcked=no
-----
<1> Numbers indicate message sequence, number of items inside each page file, number of elements in the page, and how far has Logstash progressed through the page file.
<2> Checkpoints may show `fully-acked: NO` or `fully-acked: YES`.
After all elements in the page file have been read (events ingested), the status of `fully-acked` will go from `NO` to `YES`.
...
pageNum=2, firstUnackedPageNum=2, firstUnackedSeqNum=534041, minSeqNum=457916, elementCount=76127, isFullyAcked=no
-----
<1> `NOT FOUND` is an indication of a corrupted page file.

NOTE: If the queue shows `fully-acked: YES` and 0 bytes, you can safely delete the file.  

If Logstash doesn't start and/or `pqcheck` shows an anomaly, such as `NOT_FOUND` for a page, run `pqrepair` on the queue folder.

[[pqrepair]]
===== `pqrepair` utility

The `pqrepair` utility tries to remove corrupt queue segments to bring the queue back into working order. 
It starts searching from the folder where is launched and looks for `data/queue/main`.
...
The `pqrepair` utility requires write access to the folder. 

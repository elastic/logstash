...
<<event-stats,`events`>>:: 
Gets cumulative event counts for the Logstash instance (regardless of how many
...
The following request returns a JSON document containing cumulative event counts

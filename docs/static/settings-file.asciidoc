...
| List of allowed SSL/TLS versions to use when establishing a secure connection. The available protocols depends on the JVM version, a few protocols are disabled by default and needs to be enabled manually by changing `jdk.tls.disabledAlgorithms` in the *$JDK_HOME/conf/security/java.security* configuration file.
| _N/A_

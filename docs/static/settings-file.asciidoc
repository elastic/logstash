...
Ignored unless `api.auth.type` is set to `basic`.
The default password policy requires non-empty minimum 5 char string that includes a digit, upper case letter and lower case letter.
Set `api.auth.basic.password_policy.include.symbol` `REQUIRED` to accept only passwords that include at least one special character.
Set `api.auth.basic.password_policy.length.minimum` to a value from 6 to 1024 if you want to require more than the 5 character default setting for passwords.
| `WARN`

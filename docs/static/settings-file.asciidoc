...
| Defines the behavior to apply when the dead_letter_queue.max_bytes is reached. When an event would increase the size
  over `dead_letter_queue.max_bytes` the event is dropped if valued `drop_newer` or remove the oldest segment if `drop_older`.
| `drop_newer`

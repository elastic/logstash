...
using the pipeline.id as name of the file. The destination directory is taken from the `path.logs` setting. When there are many pipelines configured in Logstash,

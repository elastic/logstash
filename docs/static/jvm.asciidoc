...
NOTE: Running {ls} on a JDK older than 21 is deprecated and will no longer be
supported in a future release. {ls} logs a deprecation warning at startup when it
runs on Java 11 or Java 17.
...
Adoptium Eclipse Temurin 21, the latest long term support (LTS) release of the JDK.
...
If you are upgrading from  {ls} 7.11.x (or earlier) to 7.12 or later, you need to update settings in `jvm.options` and
`log4j2.properties`

...
* If you are using a proxy in between the Logstash instances, the only option would be to use a TCP proxy.
* There's no explicity way to excert back-preassure back to beats input.
...
* If you are using a proxy in between the Logstash instances, any HTTP proxy can be used. 
* Additional connection information will be added to events (by the HTTP input), and this may be data you don't want.

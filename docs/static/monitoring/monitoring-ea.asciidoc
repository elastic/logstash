...
==== Install and configure {agent} to use Logstash Dashboards (Technical Preview)

Prerequisites:

* Create a fully managed Project, and select Observability
* Open the project from your cloud console
* Select observability->Add Data
* Navigate to the Integrations section at the bottom, and  click "start exploring"
...
* To collect and visualize logs, ensure the **Logs** section is enabled, and modify the log paths
to match your {ls} environment.
* Uncheck the **Metrics (Stack Monitoring)** section - this is used to send data to populate the Stack monitoring UI, and not the Logstash Dashboards
* Check the **Metrics (Technical Preview)** section. Make sure the Logstash URL setting
points to your {ls} host URL. By default, the integration collects {ls}
monitoring metrics from `http://localhost:9600`. If that host and port number are not
correct, update the `Logstash URL` setting. If you configured {ls} to use encrypted
communications, you must access it via HTTPS. For example, use a `Logstash URL` setting
like `https://localhost:9600`. If using HTTPS and/or basic auth based username and password, be sure to also fill in the advanced options appropriately. See https://www.elastic.co/guide/en/beats/metricbeat/current/configuration-ssl.html#ssl-client-config for how to configure the SSL section

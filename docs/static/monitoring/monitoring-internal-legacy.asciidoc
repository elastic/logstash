...
host, and select **Stack Monitoring** from the side navigation. If this is an initial setup, choose *Or, set up with self monitoring* option from the UI and click *Turn on monitoring* button.
Metrics reported from your Logstash nodes should be visible in the Logstash section. When security is

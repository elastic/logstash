...
monitoring cluster will result in Logstash metrics showing up in the monitoring cluster and not the production cluster. 
...
--------------------------------------------------
<1> If SSL/TLS is enabled on the production cluster, you must connect through
HTTPS. As of v5.2.1, you can specify multiple {es} hosts as an array as well as
...
<<monitoring-troubleshooting, change the password and re-enable the logstash_system user>>.

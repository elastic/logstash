...
Input plugin entry of pipeline in the API response includes `throughput` to provide visibility into the flow of events through the input plugin.

Filter and output plugin entries of pipeline in the API response include `worker_utilization` metric, indicates how many percentage of work the workers spent when proceeding event processing through the plugin and `worker_cost_per_event` metric, indicates how much time the workers spent on each event while processing through the plugin.

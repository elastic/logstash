...
Filter and output plugin entries of pipeline in the API response include `worker_utilization` and `worker_cost_per_event` <<flow-stats,_flow_ rates>>, where `worker_utilization` indicates the rate of percentage of work the workers are spending when proceeding event processing through the plugin. `worker_cost_per_event` indicates the rate how much time the workers are spending on each event while processing through the plugin.

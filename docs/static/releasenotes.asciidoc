...
=== Logstash Commits between 6.6 and 6.6.1

Computed with "git log --pretty=format:'%h -%d %s (%cr) <%an>' --abbrev-commit --date=relative v6.6.1..6.6"

bfe52a514 - (HEAD -> 6.6, origin/6.6) Convert instructions for Java plugins to asciidoc (3 hours ago) <Karen Metts>
8caaa5bfe - Update patch plugin versions in gemfile lock in preparation for 6.6.2 (#10505) (11 days ago) <João Duarte>
58590ce79 - Mute CI integration DLQ acceptance test (12 days ago) <Guy Boertje>
...
f93b4382d - update test fixture to reflect NOTICES format (#10453) (3 weeks ago) <Ry Biesemeyer>
...
Computed from "git diff v6.6.1..6.6 *.release"

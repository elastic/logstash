...
* Bootstrap air-gapped environment for GeoIP database service https://github.com/elastic/logstash/pull/13104[#13104].
  For an air-gapped environment, Users can use a script, `elasticsearch-geoip`, to bootstrap a mock server to interact with Logstash.
  Set `xpack.geoip.download.endpoint` to use the mock server.

* Fixed a shutdown error when using an external GeoIP DB https://github.com/elastic/logstash/pull/13224[#13224]

* Fixed GeoIP database service SSL verification error
  - The certificate cross-signed by the expired DST Root CA X3 failed the verification since 30/9
  - Started using Manticore (Apache HTTP client) that does not depend on jruby-openssl
...
* Our ECS efforts caused us to introduce a problem when updating some plugins depending on
  our ecs_compatibility_support helper, this is now resolved https://github.com/elastic/logstash/pull/13268[#13268]

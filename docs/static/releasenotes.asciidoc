...
* This release brings some significant improvements to helping users manage their dead letter queues, including:
 ** The introduction of a `clean_consumed` option on the Dead Letter Queue input plugin, that will automatically
    delete segments from a dead letter queue, once all their events have been consumed by a logstash pipeline.
 ** The introduction of an age retention policy, enabling the automatic removal of segments from a dead letter queue
    based on the age of events within those segments.
 ** We now expose further dead letter queue metrics on the monitoring API https://github.com/elastic/logstash/pull/14324[#14324]
* Many AWS plugins are now bundled in a single integration plugin, enabling easier maintenance and upgrades of AWS based
  plugins, which are now using version 3 of the AWS Ruby SDK.
* Logstash now comes bundled with JDK17, while still providing compatibility with user-supplied JDK11.
* Logstash is now available for download on M1 equipped MacOS devices, and come bundled with M1 native JDK17.
...
* Only remove `/etc/systemd/system/logstash.service` when file is installed by Logstash https://github.com/elastic/logstash/pull/14200[#14200]
* Fix Arcsight module compatibility with Elasticsearch `8.x` https://github.com/elastic/logstash/pull/13874[#13874]
* Ensure timestamp values are serialized with a minimum of 3 decimal places to ensure millisecond precision timestamps match those from Logstash `7.x` https://github.com/elastic/logstash/pull/14299[#14299]
...
* The version of Sinatra has been update to 2.2.2  https://github.com/elastic/logstash/pull/14454[#14454]
* The version of Nokogiri has been update to 1.13.8 https://github.com/elastic/logstash/pull/14454[#14454]
...
* Exposes metrics about segments and events cleaned by this plugin https://github.com/logstash-plugins/logstash-input-dead_letter_queue/pull/45[#45]
...
* Introduce AWS Integration Plugin. This is an integrated plugin, incorporating and replace the use of the following
  individual plugins:

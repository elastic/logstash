...
==== Logstash core
* BREAKING CHANGE: Many plugins can now be run in a mode that avoids implicit conflict with the Elastic Common Schema (ECS).
...
* BREAKING CHANGE: Setting `config.field_reference.parser` has been removed.

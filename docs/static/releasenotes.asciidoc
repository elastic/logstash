...
**Drop of Ruby Execution Engine**
The Java Execution Engine has been the default engine since Logstash 7.0, and is capable of using plugins written
in either Ruby or Java, removing the Ruby Execution Engine will not affect our ability to run existing pipelines. (#12517)

**Support for UTF-16**
 Add the support for UTF-16 and other multi-byte-character when reading log files. (#9702)
...
and rejects inputs that are either ambiguous or illegal. (#12466)

==== New features and enhancements

**Adds option to pass custom data to the benchmark CLI**
Adds another flag to the Benckmark CLI to allow passing a data file to the custom test case.
This allows to run the Benchmark CLI in a custom test case with a custom config and a custom dataset. (#12437)

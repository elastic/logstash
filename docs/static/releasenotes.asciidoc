...
Release 7.16.1 includes updated versions of filters that will not run on JDK 8.
We are actively patching the build scripts for all filters that are affected. 

Workaround: Clear the `JAVA_HOME` variable to use the JDK version (Java 11) bundled with Logstash. 

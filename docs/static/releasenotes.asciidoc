...
* <<plugins-codecs-json_lines,json_lines codec>>
...
Values for `pipeline.ordered` and `pipeline.ecs_compatibility` settings are now supported for Logstash Central Management in Kibana. https://github.com/elastic/logstash/pull/12861[#12861]
...
* Fix to log4j configuration that prevented the rollover of logstash-plain.log works when the log per pipeline (`pipeline.separate_logs`) is enabled.

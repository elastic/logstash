...
* ANNOUNCEMENT: Java 11 Support
* ANNOUNCEMENT: Native support for Java plugins (beta)
* BUGFIX: Fix issue setting 'enable_metric => false' on a plugin
* BUGFIX: Allow explicitly-specified Java codecs
* Enhancement: Update to send api version 6
* Enhancement: bump jruby to 9.2.6.0
* Cleanup: Resolve compiler and javadoc warnings
* Cleanup: Remove unused witness classes
* BUGFIX: Fix agent silent exit upon pipelines reloading
* BUGFIX: Character encoding fix
* BUGFIX: Fix crash that occurs on pipeline load and/or reload when using both the java keystore and the multi-pipeline feature, when more than one pipeline references `${}`-style variables.
* BUGFIX: Synchronise control on agent webserver
* Enhancement: Rename config option .url and .ca to .hosts and .certificate_authority, support old config options and add deprecation warnings
* BUGFIX: synchronize ruby pipeline initialization to fix concurrency bug

...
* We fixed an event serializing incompatibility introduced in 8.3.0 https://github.com/elastic/logstash/pull/14314[#14314]
  If you're using dead letter queue we recommend that you do not use Logstash 8.3.0 and upgrade to 8.3.1.
...
An event serialization bug was discovered, which causes an issue when trying to read dead letter queues created
in previous versions of Logstash.

We recommend not upgrading to Logstash 8.3.0 if you are using the dead letter queue.

...
Fixed a potential clash where `logstash-filter-date` and `logstash-filter-geoip` were used in the same pipeline. We believe
this only manifested in testing rather than actual pipeline.
Potential clash where filter-date and filter-geoip is used
...
Logstash keystore failure
Fixed a regression introduced in `7.11`  where `bin/logstash-keystore list` would not retrieve list secrets from the
logstash keystore
...
Benchmark Tool
Fixed a regression where the benchmarking tool was unable to run due to a change in the format of the stats API
...
* Updated jruby to 9.2.16.0
* Updated bundled JDK to 11.0.10+9
* Updated log4j2 to latest and ship log4j 1.2 bridge
* Updated slf4j to latest 1.7.30

=== REPEATED INFO ===
Repeated from old release notes
...
Uninstall plugin regardless of working directory

...
Fixed long-standing issue with event's count exposed in metrics when the events are dropped from pipelines https://github.com/elastic/logstash/pull/13593[#13593].
Mitigates a memory leak that can be caused by using UUIDs or other high-cardinality field names https://github.com/elastic/logstash/pull/13642[#13642].
Fixed an issue with Azure input plugin, that when used in a pipeline made Logstash to crash https://github.com/elastic/logstash/pull/13603[#13603].

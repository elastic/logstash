...
We have fixed a PQ issue where an unexpected shutdown could cause corruption of the persistent queue https://github.com/elastic/logstash/pull/12554[#12554]

**Certificate Verification with Internal Monitoring**
A bug in the monitoring pipeline was causing it to always start its Elasticsearch Output Plugin with certificate
verification disabled, which meant that Logstash Internal Monitoring has been sending _monitoring_ metadata (such as
pipeline throughput metrics) to Elasticsearch without verifying the recipient. This bug did _NOT_ affect instances of
the Elasticsearch Output Plugin in other pipelines. https://github.com/elastic/logstash/pull/12749[#12749]

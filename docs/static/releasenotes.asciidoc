...
* Fixed long-standing monitoring incompatibility on Windows where the CPU metric was not available.
...
* Recently, users running `bin/logstash-plugin` to install or update plugins stumbled upon an issue that would prevent
  Logstash from starting due a third-party dependency update.

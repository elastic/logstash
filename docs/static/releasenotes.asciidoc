...
In this release, we've made more Logstash plugins compatible with the Elastic Common Schema (ECS). This release builds on ECS work in previous release, and adds ECS-compatibility for these plugins:
...
**Potential Plugin clash fixes**

Fixed a potential clash where `logstash-filter-date` and `logstash-filter-geoip` were used in the same pipeline. 
...
previously introduced https://github.com/elastic/logstash/pull/12738[#12738]

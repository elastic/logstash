...
* Logstash should still run, and other plugin operation such as `update` and `install` should still work as expected.
** `bin/logstash-plugin list` may fail with the same error after a failed uninstallation.

...
* Fix an issue where installs and updates of certain logstash plugins may fail behind a proxy https://github.com/elastic/logstash/pull/15131[#15131]. This issue was manifesting after a recent change to `logstash-filter-translate` updated to require the use of the `jar-dependencies` gem to retrieve artifacts from maven when installing the plugin, and could prevent the update of this plugin when a proxy is in use.
* Improve logging when logstash is stalled on shutdown https://github.com/elastic/logstash/pull/15056[#15056]. This commit adds information about the main thread, if it is that one that is causing the shutdown to be stalled.
* Improve SSL settings for connection to elasticsearch for central management and monitoring https://github.com/elastic/logstash/pull/15045[#15045]. This commit adds settings support for file-based certificates and cipher suites for management and monitoring settings, and removes the deprecation warnings from the logs that have been since SSL configuration settings were revamped in the Elasticsearch output.
...
* Docs: Updated the `ssl_client_authentication` and `ssl_verify_mode` documentation explaining that CN and SAN are not validated. https://github.com/logstash-plugins/logstash-input-beats/pull/473[#473]

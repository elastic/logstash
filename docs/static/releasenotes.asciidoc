...
==== Upgrading to Logstash 7.10
Starting with this version, Logstash comes with bundled JDK. If you are upgrading from a previous version you already have
installed a JDK. In this case Logstash starts using the bundled JDK ignoring the system's one. Instead if you specified
JAVA_HOME, Logstash will pick up that JDK, but warn you.

...
* The plugin manager's `update` command now correctly only updates _minor_ versions of plugins by default to avoid breaking changes; if you wish to also include breaking changes, you must now specify `--level=major` https://github.com/elastic/logstash/pull/16975[#16975]

...
* Gauge type metrics, such as current and peak connection counts of Elastic Agent, are now available in the `_node/stats` API response when the `vertices=true` parameter is included. These metrics are particularly useful for monitoring {ls} plugin activity on the {ls} Integration dashboards [#18088](https://github.com/elastic/logstash/pull/18088)

[[featured-8-19-4]]
==== New features and enhancements

===== ES|QL Support in Elasticsearch Filter (Technical Preview)
* https://github.com/logstash-plugins/logstash-filter-elasticsearch/pull/199[Support] for using ES|QL queries in the Elasticsearch filter to add improved flexibility when ingesting data from {es} is now in Technical Preview. 

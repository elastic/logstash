...
* Fixed an issue where events passing through PQ would have their RubyStrings encoding changed. The solution involved serializing the RubyString as Java String UTF-16 encoded https://github.com/elastic/logstash/pull/9167[#9167]
...
* Ensure that metadata keys are valuefied. Before this could cause a plugin to trigger a Java exception when placing data into an event's metadata https://github.com/elastic/logstash/pull/9299[#9299]
...
* Fix correctly handle empty values between value separator and field separator (#58)

...
This section summarizes the changes in {version} release.
...
can be queried using simple APIs. Please refer <<monitoring>> for details.
...
* Breaking Change: Introduced a new way to configure application settings for Logstash through a `logstash.yml` file. This
...
been updated to use the new API, and their versions have been bumped to the next major. Please refer <<event-api>> for details.({lsissue}5141[Issue 5141])
* Breaking Change: Environment variables inside the Logstash config are evaluated by default. There is no need to specify 
`--allow-en` feature flag.
...
* Added support for `DEBUG=1` when running any plugin-related commands. This option gives you a bit more information 
about what the bundler is doing.

...
As part of the Beats refactor, we now only support private keys in PKCS#8 format. You can easily convert existing keys to use the PKCS#8 format by using the OpenSSL Toolkit. See the https://www.openssl.org/docs/man1.0.2/apps/pkcs8.html[OpenSSL] documentation.

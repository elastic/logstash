...
{ls} must establish a secure connection using an {es} self-signed SSL certificate before it can transfer data to an on-premise {es} cluster. 
{es} generates its own default self-signed Secure Sockets Layer (SSL) certificates at startup. 
{ls} must have a copy of the {es} CA and the self-signed certicate to establish a secure connection. 

Our hosted {ess} simplifies safe, secure communication between Logstash and Elasticsearch. 
{ess} uses standard publicly trusted certificates, and therefore setting a cacert is not necessary.

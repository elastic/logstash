...
You can create a pipeline and string together plugins--<<input-plugins,inputs>>, <<output-plugins,outputs>>, <<filter-plugins,filters>>, and sometimes <<codec-plugins,codecs>>--in order to process data. 
...
If you see a message in stdout that indicates that Elasticsearch is unreachable, make sure that Elasticsearch is installed and running, and is reachable on port 9200. 

...
a single host as a string, or specify multiple hosts as an array. Note that you
should route Logstash metrics through coordinating nodes, and never through
master-only nodes. Defaults to `http://localhost:9200`.

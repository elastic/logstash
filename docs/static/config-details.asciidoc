...
The total memory used by the JVM comprises native memory allocated for internal JVM data structures, thread stacks,
...
Considering a Logstash running 10 pipelines and that JVM by default allocates as much direct memory as Java heap,
we could end with:

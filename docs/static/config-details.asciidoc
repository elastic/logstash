...
* The total memory used by the JVM comprises native memory allocate for internal JVM data structures, thread stacks,
memory mapped files, the direct memory used for IO operations and the Java heap used to execute Logstash.
For example if a Logstash instance is running a certain number of  pipelines, all with PQ enabled we could
estimate a memory consumption as follow:
+
[source,text]
-------------------------------------
pipelines number x (workers x stack size + 2 * PQ page size) + direct memory + Java heap
-------------------------------------

Considering a Logstash running 10 pipelines and that by default JVM allocates as much direct memory as Java heap,
we could end in:
  * native memory: 10 x (12 x 1Mb + 128Mb), 1.4Gb
  * direct memory: 4Gb
  * Java heap: 4Gb
There is an amount of native memory, other than Java heap and direct memory, that has to be accounted to size
the memory consumption of the overall Logstash process and that could be estimated and doesn't have a direct control.
...
* If you plan to use some plugins that leverages direct memory, like Beats, TCP or HTTP input,
 ...
 they need to sustain, but never go to the maximum java heap (Xmx).

...
in particular, some input plugins such as Elastic Agent, Beats, TCP, and HTTP inputs, use direct memory.

Another component of memory consumption is the thread stack size, allocated per thread.

...
===== Age policy
To control the amount of events in a dead letter queue, it's available also the option to clean events
that are older than a certain period. If in your use case storing events older then 2 days is useless,
then you can set the `dead_letter_queue.retain.age` to automatically remove expired events.
...
The available time units are `d`, `h`, `m`, `s` respectively for day, hours, minutes and seconds.
There is no default time unit, so it has to be specified.

This policy is verified and executed on events writes and during the shutdown of a pipeline,
so your dead-letter-queue folder may store expired events for longer periods and the reader pipeline could
encounter events that should be considered outdated.

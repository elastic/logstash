...
A Logstash pipeline lets you string together plugins--<<input-plugins,inputs>>, <<output-plugins,outputs>>, <<filter-plugins,filters>>, and sometimes <<codec-plugins,codecs>>--in order to process data. 
...
When you run Logstash, you use the `-f` to specify your pipeline config file.

Let's step through creating a simple config file on your local machine and then using it to run Logstash. 
...
Note that if you see a message in stdout that reads "Elasticsearch Unreachable" that you will need to make sure Elasticsearch is installed and up and reachable on port 9200. 
Before we move on to some <<config-examples,more complex examples>>, let's take a closer look at what's in a pipeline config file.

[[configuration-file-structure]]
=== Structure of a pipeline

A Logstash pipeline config file has a separate section for each type of plugin you want to add to the event processing pipeline. 
For example:

[source,js]
----------------------------------
# This is a comment. You should use comments to describe
# parts of your pipeline configuration.
input {
  ...
}
...
by a block of settings for that plugin. 
...
A Logstash pipeline usually has three stages: inputs -> filters ->
outputs. 
Inputs generate events, filters modify them, outputs ship them
elsewhere.

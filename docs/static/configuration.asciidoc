...
[discrete]
[[date-math]]
===== Date math in conditionals

Date math in conditionals is not currently supported, but a workaround is available. 
...
----------------------------------
mutate {
  add_field => {"[@metadata][short_time]" => "%{+HH}:%{+mm}"}
  }
if [@metadata][short_time] < "15:30" {
  mutate {
    add_field => {"string_compare" => "%{+HH}:%{+mm} is before 15:30"}
    }
    }
----------------------------------
...
Another use case is the https://github.com/logstash-plugins/logstash-input-couchdb_changesCouchDB[Changes input plugin]. 

...
* Consider enabling the metering of batch sizes using the setting `pipeline.batch.metrics` to help you understand the actual batch sizes being processed by your pipeline. This setting can be useful when tuning the `pipeline.batch.size` setting. For more details see [logstash.yml](/reference/logstash-settings-file.md).

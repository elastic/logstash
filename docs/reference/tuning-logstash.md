...
* The `pipeline.batch.output_chunking.growth_threshold_factor` setting controls when output batches are split into smaller chunks. When the ratio of filtered events to input events exceeds this factor, the batch is chunked into smaller pieces based on the configured batch size. This is useful when filters significantly increase the number of events in a batch, which could otherwise cause memory pressure. The default value of 1000 effectively disables chunking; set to a lower value to enable it.

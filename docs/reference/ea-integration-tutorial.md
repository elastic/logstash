...
This tutorial walks you through adding the {integrations-docs}/docs/reference/crowdstrike-intro.md[Crowdstrike integration] sending the data to {{ech}} or self-managed {{es}}.
...
is selected and the Agent policy selected is the {{ls}} policy we created for our {{ls}} output. 
...
1. Create a new {logstash-ref}/configuration.html[{{ls}} pipeline].
...
If you run into problems, check out the [Troubleshooting](logstash-docs-md://lsr/plugins-filters-elastic_integration.md#troubleshooting) section in the `elastic_integration` filter docs.

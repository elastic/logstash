...
| `pipeline.batch.output_chunking.growth_threshold_factor` | Controls when output batches are split into smaller chunks. When the ratio of filtered events to input events exceeds this factor, the batch is chunked based on the configured batch size. Useful when filters significantly multiply the number of events in a batch. | `1000` |

...
| `ssl.reload.automatic` | Configure Logstash to automatically reload supported TLS files when they change on disk. Requires using `config.reload.automatic`. See [Automatic reload of TLS certificates](/reference/reloading-config.md#automatic-reload-of-tls-certificates). | `false` |

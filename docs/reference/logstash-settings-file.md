...
| `pipeline.batch.output_chunking.growth_threshold_factor` | Controls when output batches are split into smaller chunks. When the event count of a batch increases by this multiplication factor by the end of the pipeline's filter section, it is chunked based on the value of `pipeline.batch.size`. For example: a batch size of 50 that grows to over 50,000 events (1000x50=50,000) will be sent to outputs 50 events at a time instead of a single enormous batch. Useful when batch size grows unpredictably due to filter plugins such as split or clone. | `1000` |
...
| `ssl.reload.automatic`  {applies_to}`stack: ga 9.5.0+` | Configure Logstash to automatically reload supported TLS files when they change on disk. Requires using `config.reload.automatic`. See [Automatic reload of TLS certificates](/reference/reloading-config.md#automatic-reload-of-tls-certificates). | `false` |
...
| `otel.metrics.enabled` {applies_to}`stack: preview 9.5.0+` | Turn on or off OpenTelemetry metrics export. Refer to [Monitoring with OpenTelemetry](/reference/monitoring-with-opentelemetry.md). | `false` |
| `otel.exporter.otlp.endpoint` {applies_to}`stack: preview 9.5.0+` | OTLP metrics endpoint URL. For gRPC, typically port 4317. For HTTP, typically port 4318. When using HTTP protocol, `/v1/metrics` is automatically appended if not already present, and the default port (80 for `http://`, 443 for `https://`) is added if no port is specified. | `http://localhost:4317` |
| `otel.metric.export.interval` {applies_to}`stack: preview 9.5.0+` | Export interval with time unit. Controls how frequently metrics are sent to the OTLP endpoint. For example, `"10s"`, `"1m"`. | `"10s"` |
| `otel.exporter.otlp.protocol` {applies_to}`stack: preview 9.5.0+` | Protocol to use for OTLP export. Valid values are `grpc` or `http`. | `grpc` |
| `otel.exporter.otlp.headers` {applies_to}`stack: preview 9.5.0+` | HTTP headers as comma-separated `key=value` pairs to include in every OTLP request. Example: `Authorization=ApiKey xxx` or `Authorization=Bearer xxx,X-Custom=foo`. | *N/A* |
| `otel.resource.attributes` {applies_to}`stack: preview 9.5.0+` | Additional OpenTelemetry resource attributes as comma-separated key=value pairs. Example: `environment=production,cluster=us-west`. | *N/A* |
| `otel.service.name` {applies_to}`stack: preview 9.5.0+` | Service name for metrics. Can also be set via `otel.service.name` system property. | `logstash` |
| `otel.dataset` {applies_to}`stack: preview 9.5.0+` | Value for the `data_stream.dataset` resource attribute. When sending to the Elastic ingest endpoint, `.otel` is automatically appended — the default `logstash` produces `data_stream.dataset: logstash.otel` and index `metrics-logstash.otel-<namespace>`. This setting is only configurable via `logstash.yml` and has no corresponding system property. | `logstash` |

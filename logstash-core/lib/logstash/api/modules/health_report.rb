      ...
      class HealthReport < ::LogStash::Api::Modules::Base
        ...
        get "/" do
          payload = health_report.all.then do |health_report_pojo|
            # When invoking JrJackson from Ruby, it cannot serialize a POJO directly
            # due to the deprecated afterburner plugin failing to load a generated
            # serializer class (even though it works for POJOs that are _contained_ in
            # an IRubyObject collection).
